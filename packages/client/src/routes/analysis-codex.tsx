...
import {
    apiGetAggregatedDataKeysCodex,
    apiGetAggregatedDataPerTaskCodex,
    apiGetAggregatedDataPerUserCodex,
} from "../api/api-analysis";
import { AnalysisComponent } from "../components/analysis-component";

export const AnalysisCodexPage = () => {
    ...
    useEffect(() => {
        if (selectedTaskId !== null && selectedUserId !== null) {
            if (perTask) {
                apiGetAggregatedDataPerTaskCodex(selectedTaskId)
                    .then(async (response) => {
                        const data = await response.json();
                ...
                apiGetAggregatedDataPerUserCodex(selectedUserId)
                    .then(async (response) => {
                        const data = await response.json();
